...
configure_package() {
	...
	PKG_CONFIGURE_OPTS="--prefix=/usr \
                      ...
                      --with-libiconv=$TARGET_SYSROOT/usr \
                      --cross-compile \
...
configure_target() {
	...
	PYTHON_CONFIG="$TARGET_SYSROOT/usr/bin/python-config" \
...
makeinstall_target() {
	./buildtools/bin/waf install --destdir=$TARGET_SYSROOT --targets=smbclient -j$CONCURRENCY_MAKE_LEVEL
