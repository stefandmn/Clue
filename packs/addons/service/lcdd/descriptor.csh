...
PKG_CONFIGURE_OPTS_TARGET="--with-ft-prefix=$TARGET_SYSROOT/usr \
                           --enable-libusb \
                           --enable-libftdi \
                           --disable-libX11 \
                           --enable-libhid \
...
addon() {
	...
	cp -PR $PKG_BUILD/.install_pkg/etc/LCDd.conf ${BUILDER_ADDON}/$PKG_ADDON_ID/config/
	cp -PR $PKG_BUILD/.install_pkg/usr/lib ${BUILDER_ADDON}/$PKG_ADDON_ID/lib/
	cp -PR $PKG_BUILD/.install_pkg/usr/sbin ${BUILDER_ADDON}/$PKG_ADDON_ID/bin/

	cp -L $(get_build_dir serdisplib)/.install_pkg/usr/lib/libserdisp.so.1 ${BUILDER_ADDON}/$PKG_ADDON_ID/lib/

	sed -e "s|^DriverPath=.*$|DriverPath=/clue/.kodi/addons/service.lcdd/lib/lcdproc/|" \
