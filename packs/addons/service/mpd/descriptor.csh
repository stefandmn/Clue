...
addon() {
	...
	cp -P $PKG_BUILD/.$TARGET_NAME/mpd ${BUILDER_ADDON}/$PKG_ADDON_ID/bin
	# copy mpd cli binary
	cp -P $(get_build_dir mpd-mpc)/.$TARGET_NAME/mpc ${BUILDER_ADDON}/$PKG_ADDON_ID/bin

	mkdir -p ${BUILDER_ADDON}/$PKG_ADDON_ID/lib
	cp -p $(get_build_dir libmpdclient)/.install_pkg/usr/lib/libmpdclient.so ${BUILDER_ADDON}/$PKG_ADDON_ID/lib
	cp -p $(get_build_dir libmpdclient)/.install_pkg/usr/lib/libmpdclient.so.2 ${BUILDER_ADDON}/$PKG_ADDON_ID/lib
}
