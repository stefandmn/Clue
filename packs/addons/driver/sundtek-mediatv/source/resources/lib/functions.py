...
__sundtek_userspace__ = '/clue/.kodi/userdata/addon_data/driver.dvb.sundtek-mediatv/'
...
def settings_backup(settings_xml):
	try:
		with open(settings_xml + '_orig') as f:
			pass
	except IOError as e:
...
def settings_restore(settings_xml):
	try:
		...
		print
		'Error restoring file:', settings_xml
...
def get_devices_hdhomerun(hdhomerun_log):
	...
	try:
		...
		print
		'Error reading hdhomerun log file', hdhomerun_log
	return tuners
...
def get_devices_sundtek(mediaclient_e):
	...
	try:
		...
		print
		'Error getting sundtek tuners info'
	return tuners
...
def parse_settings(settings_xml):
	try:
		...
		print
		'Error parse settings file', settings_xml
		return None
...
def save_settings(settings_xml, xmldoc):
	try:
		...
		print
		'Error saving file:', settings_xml
